...
- `react-markdown` + `remark-gfm` — 메모 상세보기의 마크다운 렌더링 (MemoDetail 한정)
- `@google/genai` — AI 요약 기능 (서버사이드 API Route에서만 사용, 클라이언트 노출 없음)
...
| `MemoItem.tsx` | 개별 메모 카드 렌더링 (클릭 시 상세보기 호출) |
| `MemoList.tsx` | 메모 목록 및 필터/검색 UI |
| `MemoDetail.tsx` | 메모 상세보기 모달 (원본/마크다운 프리뷰 좌우 분할 뷰, AI 요약 (Gemini API Route 호출, 결과 캐싱), ESC·배경 클릭으로 닫힘) |
